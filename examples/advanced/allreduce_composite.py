...
"""L3 multi-card mesh all-reduce as a single composite call.

The companion to ``allreduce.py``, which hand-rolls the same reduction on the
L3 primitives (notify/wait barrier, then ``remote_load`` from every peer and
accumulate). Here the whole reduction is one call::

    data = pld.tensor.allreduce(data, signal, op=pld.ReduceOp.Sum, mode="mesh")

Reading the two side by side shows what the composite does on your behalf, and
what you give up in control by using it. They also barrier differently: the
hand-rolled path barriers once and then walks the payload, while the composite
barriers once per chunk.

The composite is **in-place and window-as-result**: after the call every rank's
slice of ``data`` holds the reduced value, and reading it back into ``out`` is
the caller's job. Fixed at P=2 (two ranks); mesh mode only.

Run::

    python examples/advanced/allreduce_composite.py -p a2a3 -d 0,1
    python examples/advanced/allreduce_composite.py -p a2a3 -d 0,1 --size 65536
"""
...
# Payload chunk, in elements: 4096 FP32 = 16 KiB, the granularity the mesh
# composite chunks at internally. Staging and reading back a chunk at a time
# keeps the example runnable at realistic payload sizes — a single [1, SIZE]
# tile hits the Vec limit (188,416 B on a2a3) at 128 KB/rank here.
CHUNK = min(SIZE, 4096)
if SIZE % CHUNK:
...
def reduce_step(
    ...
    """Mesh all-reduce via the composite intrinsic.

    ``my_rank`` is unused — the composite derives rank and world size from the
    comm context. It stays in the signature so this file's orchestration
    mirrors ``allreduce.py``'s, making the two easy to diff.
    """
    ...
    # One call replaces the barrier + remote_load + accumulate of allreduce.py.
    data = pld.tensor.allreduce(data, signal, op=pld.ReduceOp.Sum, mode="mesh")

    # Window-as-result: read this rank's reduced slice back into the output.
    for c0 in pl.range(0, SIZE, CHUNK):
        out[0:1, c0 : c0 + CHUNK] = data[0:1, c0 : c0 + CHUNK]
